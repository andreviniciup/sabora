const RestaurantCard = ({ restaurant, className = "" }) => {
  ...
  return (
    <div className={`bg-figma-gray rounded-2xl p-6 mb-4 border-l-4 border-orange-500 restaurant-card ${className}`}>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              {restaurant.cuisine_type && (
                <p className="text-figma-placeholder text-xs">{restaurant.cuisine_type}</p>
              )}
            </div>
          </div>
      ...
      {(restaurant.distance_formatted || restaurant.distance) && (
        <p className="text-orange-400 text-sm">{restaurant.distance_formatted || restaurant.distance}</p>
      )}
    </div>
  )
}
